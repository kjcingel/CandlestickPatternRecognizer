...
namespace Project1Fixed
{
    public class recognizer_BullishEngulfing : Recognizer
    {
        ...
        public override string usedPattern() { return "Bullish Engulfing"; }
        /// <summary>
        ...
        public override List<int> recognize(List<Candlestick> candlesticks)
        {
            ...
            for (int i = 1; i < candlesticks.Count; i++)
            {
                ...
                if (previous.IsGreenOrRed() == false && current.IsGreenOrRed() == true && previous.Close > current.Open && previous.Open < current.Close)
                {
                    result.Add(i);
                }
            }
